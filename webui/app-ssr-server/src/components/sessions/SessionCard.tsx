...
export const SessionCard: Component<SessionCardProps> = (props) => {
  ...
  const canStop = () => ['running', 'queued', 'provisioning', 'paused'].includes(session().status);
  const canCancel = () => ['queued', 'provisioning', 'running', 'paused'].includes(session().status);

  return (
    <div
      class={`bg-white border rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer ${
        props.isSelected ? 'ring-2 ring-blue-500 border-blue-500' : 'border-gray-200'
      }`}
