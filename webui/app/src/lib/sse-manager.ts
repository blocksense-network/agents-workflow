...
export function subscribeToSession(
  ...
  console.log(`[SSEManager] Session ${sessionId} now has ${subscription.callbacks.size} subscribers`);
  ...
  return () => {
    ...
    if (sub) {
      ...
      if (sub.callbacks.size === 0) {
        console.log(`[SSEManager] No more subscribers for session ${sessionId} - closing EventSource`);
