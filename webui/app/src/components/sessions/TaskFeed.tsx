...
export const TaskFeed: Component<TaskFeedProps> = (props) => {
  ...
  const [clientDrafts, setClientDrafts] = createSignal(props.initialDrafts || []);
  ...
  const [clientSessions, setClientSessions] = createSignal(props.initialSessions || {
    items: [],
                    ...
                    <div id={`draft-task-${draft.id}`}>
                      <DraftTaskCard
                        ...
                        if (success) {
                          ...
                          setClientDrafts(prev => prev.map(d =>
            ...
            <div class={drafts().length > 0 ? "mt-6" : ""}>
              <ul role="list" class="space-y-3">
                <For each={sessionsData()?.items}>
                  {(session, index) => {
                    ...
                    return (
                      <li id={`task-${session.id}`}>
                        <SessionCard
                          session={session}
                          isSelected={selectedSessionId() === session.id || keyboardSelectedIndex() === globalIndex}
              ...
              >
                {(() => {
                  ...
                  } else if (idx >= draftCount && sessionsData()?.items[idx - draftCount]) {
