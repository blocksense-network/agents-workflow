...
type ActivityRow = 
  ...
  | { type: 'tool'; name: string; lastLine?: string; output?: string; status?: string }
...
const formatDate = (dateString: string) => {
  try {
    // Use ISO format to avoid SSR/client hydration mismatches
...
export const SessionCard: Component<SessionCardProps> = (props) => {
  ...
  onMount(() => {
    ...
    const isActive = ["running", "queued", "provisioning", "paused", "resuming", "stopping"].includes(session().status);
    if (!isActive) return;
    ...
    const unsubscribe = subscribeToSession(
      ...
      (event: SessionEvent) => {
        ...
        if (event.thought) {
          ...
          console.log(`[SessionCard ${session().id}] Added thinking row, total rows: ${newRows.length}`);
        }
        ...
        // Tool start - adds new row, scrolls up, tracks tool name
          ...
          console.log(`[SessionCard ${session().id}] Added tool start row: ${event.tool_name}, total rows: ${newRows.length}`);
        }
        ...
        // Tool last_line - updates current tool row IN PLACE, no scroll
          ...
          setLiveActivity({ ...current, rows: newRows });
          console.log(`[SessionCard ${session().id}] Updated tool last_line IN PLACE for: ${current.currentTool}`);
        }
        ...
        // Tool complete - replaces tool row with completion, clears currentTool
          ...
          console.log(`[SessionCard ${session().id}] Tool completed: ${event.tool_name}, cleared currentTool`);
        }
        ...
        // File edit event - adds new row, scrolls up
          ...
          const newRows = [...current.rows, newRow].slice(-3); // Keep last 3
          setLiveActivity({ ...current, rows: newRows });
          console.log(`[SessionCard ${session().id}] Added file edit row, total rows: ${newRows.length}`);
        }
        
        console.log(`[SessionCard ${session().id}] Live activity rows:`, liveActivity().rows.length);
      }
    );
    ...
    >
      ...
      <div class="flex items-center justify-between mb-2">
        <div class="flex items-center space-x-2 flex-1 min-w-0">
          <span class={`text-sm ${statusInfo().color}`} aria-label={`Status: ${sessionStatus()}`}>
            {statusInfo().icon}
          </span>
          <a
            href={`/tasks/${session().id}`}
            class="text-sm font-medium text-gray-900 hover:text-blue-600 hover:underline truncate cursor-pointer focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500"
      ...
      <div class="flex items-center space-x-1.5 mb-2 text-xs text-gray-600">
        <span>📁</span>
        <span class="truncate max-w-[120px]">{getRepoName(session().repo.url)}</span>
        ...
        <span class="text-gray-400">•</span>
        <span>🤖</span>
        <span class="truncate">{session().agent.type} v{session().agent.version}</span>
        <span class="text-gray-400">•</span>
        <span>🕒</span>
      ...
      <Show when={["running", "queued", "provisioning", "paused", "resuming", "stopping"].includes(sessionStatus())}>
