//! Agents-Workflow specific multiplexer abstractions
//!
//! This crate provides AW-specific functionality on top of the low-level
...
/// AW-specific multiplexer adapter
...
impl<M: Multiplexer> AwMultiplexer<M> {
    ...
    /// Create a standard AW task layout with editor (left) and agent (right) panes
    pub fn create_task_layout(&self, config: &LayoutConfig) -> Result<LayoutHandle, AwMuxError> {
        let title = format!("aw-task-{}", config.task_id);
    ...
    pub fn find_task_layout(&self, task_id: &str) -> Result<Option<LayoutHandle>, AwMuxError> {
        let title_substr = format!("aw-task-{}", task_id);
