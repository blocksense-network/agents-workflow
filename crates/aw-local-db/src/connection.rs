...
impl Database {
    /// Get the default database path based on AW_HOME environment variable or platform defaults.
    ///
    /// Priority order (per State-Persistence.md):
    /// 1. AW_HOME environment variable (custom)
    ...
    pub fn default_path() -> crate::Result<PathBuf> {
        // Check AW_HOME first
        if let Ok(aw_home) = std::env::var("AW_HOME") {
            return Ok(PathBuf::from(aw_home).join("state.db"));
        }
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            Ok(xdg_state_home.join("agents-workflow").join("state.db"))
        }
        ...
        #[cfg(not(any(target_os = "linux", target_os = "macos", target_os = "windows")))]
        {
            ...
            let home = std::env::var("HOME").expect("HOME environment variable not set");
            Ok(PathBuf::from(home).join(".agents-workflow").join("state.db"))
        }
    }
